...
const KundaliResult: React.FC<KundaliResultProps> = ({ data, name, language, onBack, formInput }) => {
  ...
  // Debug logging
  React.useEffect(() => {
    console.log("KundaliResult data:", data);
    console.log("Planetary positions:", data.charts?.planetaryPositions);
    console.log("Ascendant Sign ID:", data.basicDetails?.ascendantSignId);
    console.log("D1 Chart SVG available:", !!data.charts?.d1ChartSvg, "Length:", data.charts?.d1ChartSvg?.length);
    console.log("D9 Chart SVG available:", !!data.charts?.d9ChartSvg, "Length:", data.charts?.d9ChartSvg?.length);
  }, [data]);
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 pb-12 animate-fade-in-up">
      <div className="flex justify-between items-center mb-6 flex-wrap gap-3">
        ...
        <a
          ...
          onClick={(e) => {
            ...
            // Navigate to learning center with kundali tab
            const event = new CustomEvent('navigateToLearning', { detail: { tab: 'kundali' } });
            window.dispatchEvent(event);
          }}
          className="inline-flex items-center gap-2 px-4 py-2 bg-amber-600/20 hover:bg-amber-600/30 border border-amber-500/30 rounded-full text-xs font-bold text-amber-300 uppercase tracking-wider transition-all"
        >
          <span>📚</span>
      ...
      <div className="space-y-8">
        ...
        {/* Western Astrology View - Sun/Moon/Rising (Co-Star style) */}
        <div className="bg-gradient-to-r from-indigo-900/20 to-purple-900/20 border border-indigo-500/30 rounded-2xl p-6">
          <h3 className="text-lg font-serif text-indigo-200 mb-4 flex items-center gap-2">
            <span>♈</span>
            {language === 'hi' ? 'पश्चिमी ज्योतिष दृश्य' : 'Western Astrology View'}
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-slate-900/50 rounded-xl p-4 text-center border border-indigo-500/20">
              <span className="text-[10px] uppercase text-slate-500 tracking-widest block mb-1">{language === 'hi' ? 'सूर्य राशि' : 'Sun Sign'}</span>
              <span className="text-xl font-serif text-amber-300">{data.basicDetails?.sunSign || '-'}</span>
            </div>
            <div className="bg-slate-900/50 rounded-xl p-4 text-center border border-indigo-500/20">
              <span className="text-[10px] uppercase text-slate-500 tracking-widest block mb-1">{language === 'hi' ? 'चंद्र राशि' : 'Moon Sign'}</span>
              <span className="text-xl font-serif text-indigo-300">{data.basicDetails?.moonSign || '-'}</span>
            </div>
            <div className="bg-slate-900/50 rounded-xl p-4 text-center border border-indigo-500/20">
              <span className="text-[10px] uppercase text-slate-500 tracking-widest block mb-1">{language === 'hi' ? 'उदय राशि (लग्न)' : 'Rising Sign (Lagna)'}</span>
              <span className="text-xl font-serif text-purple-300">{data.basicDetails?.ascendant || '-'}</span>
            </div>
          </div>
          <p className="text-slate-500 text-xs mt-3 text-center">
            {language === 'hi' ? 'वैदिक कुंडली में सूर्य, चंद्र और लग्न पश्चिमी ज्योतिष में सूर्य, चंद्र और उदय राशि के समान हैं।' : 'In Vedic Kundali, Sun, Moon and Lagna correspond to Western Sun, Moon and Rising signs.'}
          </p>
        </div>
