...
**Publisher Center logos**

- **Square logo**: Use `public/app-logo.png` (512×512). Recommended: 1000×1000 PNG if you have a higher-res export.
- **Rectangular logo**: Use `public/cosmicjyoti-logo-rectangular.svg` (400×80). Google requires **PNG** for rectangular:
  - Run `npm run logo:rectangular` to generate `public/cosmicjyoti-logo-rectangular.png` (requires `sharp`).
  - Or open the SVG in a browser, then save as PNG / use an online converter (e.g. cloudconvert.com) at 400px width.

**Uploaded fonts (optional)**

- CosmicJyoti uses **Cinzel** (serif, headings) and **Lato** (sans, body), both from **Google Fonts**.
- You **do not need to upload** these in Publisher Center: they are already in Google’s font library. Leave “Upload fonts” empty unless you use a **custom/proprietary** font (not on Google Fonts) and want it used in Google-rendered CTAs; then upload the font file only if your license allows use in Google products.
...
- [x] **News sitemap**: `blog/sitemap-news.xml` with `news:publication_date`, `news:title`, `news:publication`; last 48 hours. *(Generated by generate-daily-blog.mjs when you run a slot, or run `npm run blog:refresh-sitemaps` to create/update it from existing daily-posts.json without calling the API. Then run `npm run build` and deploy so the file is live at https://www.cosmicjyoti.com/blog/sitemap-news.xml.)*
