...
export const generatePalmInterpretation = async (lines: string[], language: Language): Promise<string> => 
    generateInterpretativeReading(`Analyze these detected palm lines: ${lines.join(", ")}.`, "Act as an expert Vedic Palmist.", GEMINI_FLASH_MODEL, language);
...
export const generateConjunctionAnalysis = async (planets: string[], house: number, sign: string, language: Language): Promise<string> => {
    ...
    return generateInterpretativeReading(`Analyze the conjunction of ${planetList} in house ${house} within the sign of ${sign}.`, "Focus on Yoga results in Vedic Astrology.", GEMINI_FLASH_MODEL, language);
};

export const generateVastuAnalysis = async (room: string, direction: string, language: Language): Promise<string> => 
    generateInterpretativeReading(`Analyze the Vastu energy flow of a ${room} placed in the ${direction} direction.`, "Act as a Vastu Shastra expert.", GEMINI_FLASH_MODEL, language);

export const generateGemstoneAdvice = async (planet: string, problem: string, language: Language): Promise<string> => 
    generateInterpretativeReading(`Suggest a gemstone remedy for a weak ${planet} and the life objective: ${problem}.`, "Focus on Ratna Shastra (Gemstone Science).", GEMINI_FLASH_MODEL, language);

export const generateDreamAnalysis = async (dream: string, language: Language): Promise<string> => 
    generateInterpretativeReading(`Interpret this dream from a Vedic perspective: "${dream}".`, "Focus on Swapna Shastra (Dream Interpretation).", GEMINI_FLASH_MODEL, language);

export const generateRudrakshAdvice = async (problem: string, language: Language): Promise<string> => 
    generateInterpretativeReading(`Identify the best Rudraksh bead for this problem: ${problem}.`, "Focus on Rudraksh Vidya.", GEMINI_FLASH_MODEL, language);

export const searchOccultVault = async (query: string, category: string, language: Language): Promise<string> => 
    generateInterpretativeReading(`Explain "${query}" within the context of "${category}".`, "Provide deep esoteric wisdom.", GEMINI_FLASH_MODEL, language);

export const generateAstroStory = async (target: string, type: string, language: Language): Promise<string> => 
    generateInterpretativeReading(`Tell a mystical legend from the Puranas about the ${type} ${target}.`, "Act as a Vedic Storyteller.", GEMINI_FLASH_MODEL, language);

export const generateTarotReading = async (cards: string[], language: Language): Promise<string> => 
    generateInterpretativeReading(`Interpret this 3-card Tarot spread: ${cards.join(", ")} representing Past, Present, and Future.`, "Act as a Mystic Tarot Reader.", GEMINI_FLASH_MODEL, language);
...
async function generateKundaliDirect(formData: KundaliFormData, language: Language = 'en'): Promise<KundaliResponse> {
    try {
        ...
        if (d9ChartResponse.ok) {
            ...
            if (typeof d9ChartData === 'string' && d9ChartData.trim().startsWith('<svg')) {
                d9ChartSvg = d9ChartData.trim();
            } else {
...
const TRANSIT_SIGN_MAP: { [key: string]: number } = {
    'Aries': 1, 'Taurus': 2, 'Gemini': 3, 'Cancer': 4,
    'Leo': 5, 'Virgo': 6, 'Libra': 7, 'Scorpio': 8,
    'Sagittarius': 9, 'Capricorn': 10, 'Aquarius': 11, 'Pisces': 12
};
const TRANSIT_SIGN_NAMES = ['Aries', 'Taurus', 'Gemini', 'Cancer', 'Leo', 'Virgo', 'Libra', 'Scorpio', 'Sagittarius', 'Capricorn', 'Aquarius', 'Pisces'];

function normalizeTransitPositions(positions: any[], rashi: string): PlanetaryPosition[] {
    const referenceSignId = TRANSIT_SIGN_MAP[rashi] || 1;
    return (positions || []).map((p: any) => {
        let signId = typeof p.signId === 'number' ? p.signId : parseInt(String(p.signId), 10);
        if (!Number.isFinite(signId) || signId < 1 || signId > 12) signId = (p.sign && TRANSIT_SIGN_MAP[p.sign]) || 1;
        let house = typeof p.house === 'number' ? p.house : parseInt(String(p.house), 10);
        if (!Number.isFinite(house) || house < 1 || house > 12) house = ((signId - referenceSignId + 12) % 12) + 1;
        const sign = p.sign || TRANSIT_SIGN_NAMES[signId - 1] || 'Aries';
        const isRetro = p.planet && String(p.planet).toLowerCase().includes('moon') ? false : !!p.isRetrograde;
        return {
            planet: p.planet || '',
            sign,
            signId,
    ...
    if (hasPerplexityKey()) {
        try {
            ...
            if (perplexityResponse?.currentPositions?.length > 0) {
                const processed = normalizeTransitPositions(perplexityResponse.currentPositions, rashi);
                if (processed.length > 0) {
                    return {
                        currentPositions: processed,
                        personalImpact: Array.isArray(perplexityResponse.personalImpact) ? perplexityResponse.personalImpact : generateTransitPredictions(processed, null, language)
                    };
                }
            }
        } catch (_) {
            // fall through to backend then Gemini
        }
    }

    // 2) Try backend (no API key needed)
    try {
        const backendResponse = await generateGenericTransitsFromBackend(location, rashi, todayStr, language);
        if (backendResponse?.currentPositions?.length > 0) {
            const processed = normalizeTransitPositions(backendResponse.currentPositions, rashi);
            if (processed.length > 0) {
                return {
                    currentPositions: processed,
                    personalImpact: generateTransitPredictions(processed, null, language)
                };
            }
        }
    } catch (e: any) {
        const isNetwork = e?.message?.includes('Failed to fetch') || e?.message?.includes('timed out') || e?.code === 'ECONNREFUSED' || e?.code === 'ETIMEDOUT';
        if (!isNetwork) console.warn('Transits backend error:', e?.message);
    }
...
Language: ${language}.`;
    ...
    const response = await ai.models.generateContent({
        ...
        config: {
            ...
            if (parsed.currentPositions && Array.isArray(parsed.currentPositions)) {
                ...
                parsed.currentPositions = parsed.currentPositions.map((p: any) => {
                    ...
                    if (!p.signId || p.signId < 1 || p.signId > 12) {
                        ...
                        if (p.sign && TRANSIT_SIGN_MAP[p.sign]) {
                            p.signId = TRANSIT_SIGN_MAP[p.sign];
                        } else {
                    ...
                    if (!p.sign || TRANSIT_SIGN_MAP[p.sign] !== p.signId) {
                        p.sign = TRANSIT_SIGN_NAMES[p.signId - 1] || 'Aries';
                    }
            ...
            if (!Array.isArray(parsed.personalImpact) || parsed.personalImpact.length === 0) {
                parsed.personalImpact = generateTransitPredictions(parsed.currentPositions, null, language);
            }
            return { currentPositions: parsed.currentPositions, personalImpact: parsed.personalImpact };
        }
        )();
        ...
        throw new Error("We couldn't load transit data right now. Please try again.");
    }
};
